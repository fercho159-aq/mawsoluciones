...
const sectionIcons: Record<number, React.ReactNode> = {
  ...
  2: <SlidersHorizontal className="w-5 h-5 mr-3 text-primary" />,
  3: <Link className="w-5 h-5 mr-3 text-primary" />,
  4: <BrainCircuit className="w-5 h-5 mr-3 text-primary" />,
};
...
export default function CoursePage() {
  ...
  const handleQuizLeadSubmit = (data: QuizLeadFormData) => {
      const message = `
*¡Hola! He completado un examen del curso de Automatización con n8n.*
