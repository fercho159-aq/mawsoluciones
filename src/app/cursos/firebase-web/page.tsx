...
import { courseData } from '@/lib/firebase-web-course-data';
import type { Topic, Question } from '@/lib/course-data';
import { Progress } from "@/components/ui/progress";
import { CheckCircle, Circle, PlayCircle, Database, Lock, Cloud, FunctionSquare, Calendar, ChevronRight, X, AlertTriangle, Frown } from "lucide-react";
import {
  Accordion,
  AccordionContent,
  AccordionItem,
...
const sectionIcons: Record<number, React.ReactNode> = {
  1: <Database className="w-5 h-5 mr-3 text-primary" />,
  2: <Lock className="w-5 h-5 mr-3 text-primary" />,
  3: <Cloud className="w-5 h-5 mr-3 text-primary" />,
  4: <FunctionSquare className="w-5 h-5 mr-3 text-primary" />,
};
...
export default function CoursePage() {
  ...
  const handleQuizLeadSubmit = (data: QuizLeadFormData) => {
      const message = `
*¡Hola! He completado un examen del curso de Sitios Web con Firebase.*
                                    ...
                                    <AccordionItem value={`section-${section.section_id}`} key={section.section_id}>
                                        <AccordionTrigger className="font-headline text-lg hover:no-underline">
                                           <div className="flex items-center">
                                                {sectionIcons[section.section_id as keyof typeof sectionIcons] || <Database className="w-5 h-5 mr-3 text-primary" />}
