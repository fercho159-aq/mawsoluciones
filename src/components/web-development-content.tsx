...
const WebDevelopmentContent = ({ 
    ...
    showPortfolio = false,
    isFullPage = false
}: WebDevelopmentContentProps) => {

  const SectionWrapper = ({ children, id }: { children: React.ReactNode, id: string }) => {
    if (isFullPage) {
        return <section id={id}>{children}</section>;
    }
    return <motion.div id={id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>{children}</motion.div>;
  }
  ...
  return (
    <>
      ...
      {showInfo && (
       ...
       {showCalculator && (
        <SectionWrapper id="calculator-section">
      ...
      {showAnalyzer && (
        <SectionWrapper id="analyzer-section">
            <section className="py-20 md:py-28">
                <div className="container mx-auto px-4 md:px-6">
